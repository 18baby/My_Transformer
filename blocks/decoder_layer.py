...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, ffn_hidden, n_head, drop_prob):
        ...
        self.mask_attention = MultiHeadAttention(d_model=d_model, n_head=n_head)
    ...
    def forward(self, enc, dec, trg_mask, src_mask):
        ...
        # ===== Layer 1 =====
        # masked mulihead attention
        _x = dec
        x = self.mask_attention(q=dec, k=dec, v=dec, mask=trg_mask)
        ...
        if enc is not None:
            # mulihead attention
            _x = x
            x = self.attention(q=x, k=enc, v=enc, mask=src_mask)
